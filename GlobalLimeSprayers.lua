       ...
       -- ==========================================
        ...
        if g_storeManager ~= nil then
            for _, item in pairs(g_storeManager:getItems()) do
                -- Store items cache their specs in an array, not as direct dictionary keys.
                if item.specs ~= nil then
                    for _, spec in pairs(item.specs) do
                        -- Find the spec object responsible for fill types
                        if spec.name == "fillTypes" and type(spec.value) == "table" then
                            local supportsSprayerFluids = false
                            
                            -- Check if it supports fertilizer or herbicide
                            for _, fillTypeIndex in pairs(spec.value) do
                            ...
                            if supportsSprayerFluids then
                                ...
                                if not alreadyHasLime then
                                    table.insert(spec.value, limeType.index)
                                end
                            end
                        end
                    end
